...
patient = Entity(
    name="patient_id", value_type=ValueType.INT64, description="ID of the patient"
)

# Predictors Feature View
file_source = FileSource(
    path=r"data/predictors_df.parquet", event_timestamp_column="event_timestamp",
)

df1_fv = FeatureView(
    name="predictors_df_feature_view",
    ttl=timedelta(seconds=86400 * 10),
    entities=[patient],
    schema=[
...
target_source = FileSource(
    path=r"data/target_df.parquet", event_timestamp_column="event_timestamp",
)

target_fv = FeatureView(
    name="target_df_feature_view",
    ttl=timedelta(days=12),
    entities=[patient],
    schema=[Field(name="Outcome", dtype=Int64),],
